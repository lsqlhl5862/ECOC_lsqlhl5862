...
def getAccuracyByEncoding(dataName, encoding, trainX, trainY, testX, testY):
    ...
    for row in codeMatrix.transpose():
        ...
        for item in row:
            if item==-1 or item==1:
                validationCol.append(1)
            else :
                validationCol.append(-1)
        # print(validationCol)
        y.append((index, getDataComplexitybyCol(codeMatrix.transpose()[index], trainX, trainY),
        getDataComplexitybyCol(validationCol, trainX, trainY), k))
